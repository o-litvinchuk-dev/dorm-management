...
const presetSchema = Joi.object({
    ...
    academic_year: Joi.string().pattern(/^\d{4}-\d{4}$/).required().messages({ // e.g. 2024-2025
...
export const createPreset = async (req, res) => {
    try {
        ...
        // Dean can only create for their faculty or global if superadmin/admin
...
export const getPresetForDormitory = async (req, res) => {
    try {
        ...
        const { academic_year: academicYearQuery } = req.query; // student will provide this
        ...
        const preset = await DormitoryApplicationPreset.findByDormitoryAndAcademicYear(dormitoryId, academic_year);
        if (!preset) {
            // It's okay not to find a preset, the form will just not be pre-filled
...
export const updatePreset = async (req, res) => {
    try {
        ...
        if (req.user.role === 'faculty_dean_office') {
            ...
            // Dean cannot change a global preset to be faculty specific, or change faculty of a preset.
            // They also cannot change a faculty-specific preset to global if they are not admin/superadmin
            if (value.faculty_id !== existingPreset.faculty_id) {
                 return res.status(403).json({ error: "Деканат не може змінювати приналежність налаштування до факультету." });
            }
        }
