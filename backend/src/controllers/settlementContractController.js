...
// Helper to ensure undefined or empty strings become null for DB
...
const settlementContractSchema = Joi.object({
  ...
  day: Joi.string().length(2).pattern(/^\d+$/).required().messages({ ...joiErrorMessages, 'string.pattern.base': 'День додатку: XX' }),
  month: Joi.string().length(2).pattern(/^\d+$/).required().messages({ ...joiErrorMessages, 'string.pattern.base': 'Місяць додатку: XX' }),
  year: Joi.string().custom(shortYearDateValidation, 'YY date validation').required().messages(joiErrorMessages),
  ...
  dormNumber_appendix1: Joi.string().pattern(/^\d+$/).required().messages(joiErrorMessages),
...
export const createSettlementContract = async (req, res) => {
  try {
    // Joi validation using the schema defined above
    const { error, value: contractDataFromRequest } = settlementContractSchema.validate(req.body, {
      abortEarly: false,
      stripUnknown: true, // Important to remove any extra fields not defined in schema
    });
    ...
    if (!facultyFromDb) {
      return res.status(400).json({ // Changed to 400 as it's client-provided invalid ID
    ...
    const payloadToModel = {
      ...
      day: restOfValidatedData.day, 
      month: restOfValidatedData.month, 
      year: restOfValidatedData.year, 
      ...
      dormNumber: restOfValidatedData.dormNumber, // This should be the ID of the dormitory
      ...
      fullName_encrypted: restOfValidatedData.fullName,
      passportSeries_encrypted: restOfValidatedData.passportSeries,
      passportNumber_encrypted: restOfValidatedData.passportNumber,
      passportIssued_encrypted: restOfValidatedData.passportIssued,
      taxId_encrypted: JSON.stringify(restOfValidatedData.taxId || []),
      residentFullName_encrypted: restOfValidatedData.residentFullName,
      residentPhone_encrypted: restOfValidatedData.residentPhone,
      parentFullName_encrypted: restOfValidatedData.parentFullName,
      motherPhone_encrypted: ensureNullForDB(restOfValidatedData.motherPhone),
      fatherPhone_encrypted: ensureNullForDB(restOfValidatedData.fatherPhone),

      roomNumber: ensureNullForDB(restOfValidatedData.roomNumber),
      dorm_manager_name: ensureNullForDB(restOfValidatedData.dormManagerName_appendix1), // Using appendix1 name as main
      resident_name: ensureNullForDB(restOfValidatedData.residentName_appendix1), // Using appendix1 name as main
      ...
      premisesConditions: (restOfValidatedData.premisesConditions || []).map(item => ({
        ...
        condition: ensureNullForDB(item.condition), // Joi uses 'condition'
      })),
    ...
    if (!res.headersSent) {
      res.status(500).json({
        ...
        details: error.message, // Send back actual error message in dev, or generic in prod
      });
    }
  }
};
...
// The rest of your controller functions (getSettlementAgreementsAdmin, etc.)
// ... (getSettlementAgreementsAdmin, getSettlementAgreementByIdAdmin, updateSettlementAgreementAdmin)
// Ensure they remain as they were if they were working correctly.
// The key fix was in the Joi schema messages and data preparation in createSettlementContract.
...
export const getSettlementAgreementsAdmin = async (req, res) => {
    try {
      ...
      // Decryption should happen here after fetching
      const decryptedAgreements = result.agreements.map((agreement) => {
          // Assuming decryptContractData is correctly defined elsewhere or in this file
          const decrypt = require("../utils/crypto.js").decrypt; // Adjust path if needed
          const SENSITIVE_FIELDS_FOR_DECRYPT = [ // Ensure this list is accurate
          ...
          SENSITIVE_FIELDS_FOR_DECRYPT.forEach(key => {
              if (agreement[key]) {
                  ...
                  if (key !== decryptedKey) delete decrypted[key]; // remove encrypted field
              } else {
                decrypted[key.replace('_encrypted', '')] = null; // If no encrypted value, set decrypted to null
              }
          });
          return decrypted;
      });
  ...
  export const getSettlementAgreementByIdAdmin = async (req, res) => {
    try {
      ...
      const decrypt = require("../utils/crypto.js").decrypt; // Adjust path
