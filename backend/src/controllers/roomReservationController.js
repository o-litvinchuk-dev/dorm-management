...
const checkRoomAvailabilityForReservation = async (
  ...
  let query = `
    ...
    AND (
      (reservation_start_date < ? AND reservation_end_date > ?) OR
      (reservation_start_date >= ? AND reservation_start_date < ?) OR
      (reservation_end_date > ? AND reservation_end_date <= ?) OR
  ...
  const [conflictingReservations] = await connection.query(query, params);
  return conflictingReservations.length === 0;
};

const getDatesFromAcademicYear = (academicYear) => {
  if (!academicYear || !/^\d{4}-\d{4}$/.test(academicYear)) {
    const today = new Date();
    const currentMonth = today.getMonth();
    let academicStartYear = today.getFullYear();
    let yearForStartDate = currentMonth >= 6 ? today.getFullYear() : today.getFullYear() -1;
    yearForStartDate +=1;
    console.warn(`[getDatesFromAcademicYear] Invalid academic year format: ${academicYear}. Defaulting to next academic year relative to today.`);
    return {
      date_from: `${yearForStartDate}-09-01`,
      date_to: `${yearForStartDate + 1}-06-30`,
    };
  }
  const [startYear, endYear] = academicYear.split("-");
  return {
    date_from: `${startYear}-09-01`,
...
export const searchAvailableRooms = async (req, res) => {
  try {
    ...
    const { date_from, date_to } = getDatesFromAcademicYear(
      value.academic_year_for_search
    );
    ...
    for (const room of candidateRooms) {
      const isTrulyAvailableOnDates =
        await checkRoomAvailabilityForReservation(
          room.id,
          date_from,
          date_to
        );

      if (isTrulyAvailableOnDates) {
...
export const getRoomDetails = async (req, res) => {
    try {
      ...
      let isAvailableOnDates = true;
      let datesForCheck = {};
      if (academicYearQuery) {
         datesForCheck = getDatesFromAcademicYear(academicYearQuery);
        if (datesForCheck.date_from && datesForCheck.date_to) {
          isAvailableOnDates = await checkRoomAvailabilityForReservation(
            roomId,
            datesForCheck.date_from,
            datesForCheck.date_to
          );
        }
      }

      res.json({ ...room, isAvailable: isAvailableOnDates });
    } catch (error) {
...
export const reserveRoom = async (req, res) => {
  ...
  try {
    ...
    if (userGender === 'not_specified' || userGender === 'other') {
        await connection.rollback();
        return res.status(400).json({ error: "Будь ласка, вкажіть вашу стать у профілі для бронювання кімнати." });
    }
    ...
    const { date_from: reservation_start_date, date_to: reservation_end_date } =
      getDatesFromAcademicYear(value.academic_year);
    ...
    const isAvailableOnDates = await checkRoomAvailabilityForReservation(
      roomId,
    ...
    if (!isAvailableOnDates) {
      ...
      return res.status(409).json({
        error:
          "Ця кімната вже заброньована (підтверджено) на вибрані дати.",
      });
    }

    const [userActiveReservations] = await connection.query(
      `SELECT id FROM room_reservations WHERE user_id = ? AND status IN ('pending_confirmation', 'confirmed', 'checked_in')
    ...
    if (userActiveReservations.length > 0) {
      ...
      return res.status(409).json({
        error:
          "У вас вже є активне бронювання або заявка на поселення.",
      });
    }
    ...
    await Notification.create({
      ...
      description: `Ваше бронювання кімнати №${room.number} (ID: ${reservationId}) на ${value.academic_year} н.р. очікує підтвердження.`,
    });
    ...
    for (const manager of dormManagers) {
      await Notification.create({
        ...
        description: `Студент ${req.user.name || req.user.email
          ...
          } (ID: ${roomId}) на ${value.academic_year} н.р.`,
      });
    }
    ...
    res.status(201).json({
      message:
        "Запит на бронювання кімнати успішно створено. Очікуйте підтвердження.",
      reservationId,
    });
  } catch (error) {
    await connection.rollback();
  ...
  try {
    ...
    if (
      ...
      return res.status(400).json({
        error:
          "Можна скасувати лише бронювання, що очікують підтвердження або вже підтверджені (до фактичного заселення).",
      });
    }
    ...
    await Notification.create({
      ...
      description: `Ваше бронювання кімнати №${reservation.room_number || reservation.room_id
        } (ID: ${reservationId}) було успішно скасовано.`,
    });
...
export const updateReservationStatusAdmin = async (req, res) => {
  ...
  try {
    ...
    let studentNotificationDesc = `Статус вашого бронювання кімнати №${room.number
      ...
      }) змінено на "${statusLabels[status] || status}".`;
    if (notes_admin) {
