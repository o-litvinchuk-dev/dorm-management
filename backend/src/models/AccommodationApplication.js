...
class AccommodationApplication {
    ...
    static async findById(id) {
        ...
        try {
            ...
            // Removed the fragile academic_year matching.
            // Now it finds the latest active/confirmed reservation for the same user and dormitory.
        ...
        SELECT
            ...
            COALESCE(
                (
                    ...
                    AND rr.academic_year = CONCAT(YEAR(aa.start_date), '-', YEAR(aa.end_date)) 
                    AND rr.status IN ('confirmed', 'checked_in') 
                    ORDER BY rr.created_at DESC 
                    LIMIT 1
                ),
                (
                    SELECT COALESCE(r.number, dp.room_number_text)
                    FROM dormitory_passes dp
                    LEFT JOIN rooms r ON dp.room_id = r.id
                    WHERE dp.user_id = aa.user_id
                    AND dp.source_id = aa.id
                    AND dp.source_type = 'application'
