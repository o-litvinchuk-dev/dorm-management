...
api.interceptors.response.use(
  ...
  async (error) => {
    ...
    // Змінено: Додано перевірку, щоб не показувати помилку 404 для перепустки
    const isMyPassNotFound = statusCode === 404 && originalRequest.url.includes('/secure/my-pass');

    if (!originalRequest._noAuth && statusCode !== 401 && statusCode !== 403 && !isMyPassNotFound) {
