...
const phoneSchema = Yup.string()
  .matches(/^\d{9}$/, "Телефон: XXXXXXXXX (9 цифр)")
  .required("Телефон обов'язковий");

const surnameSchema = Yup.string()
  .matches(/^[А-ЯІЇЄҐЁ][а-яіїєґё'-]*$/, "Прізвище: з великої літери, кирилиця")
  .required("Прізвище обов'язкове");

export const fullSchema = (validGroupIds = []) =>
  Yup.object().shape(
    {
      faculty: Yup.string().required("Факультет обов'язковий"),
      group: Yup.string().when('faculty', {
        is: (val) => !!val,
        then: (schema) =>
          schema
            .test(
      ...
      academicYearEnd: academicYearPartSchema
        .test(
          'is-greater-or-equal',
          'Кінцевий рік має бути більшим або рівним початковому',
          function (value) {
        ...
        .test(
          'is-sequential',
          'Навчальний рік має бути послідовним (напр. 2024-2025)',
          function (value) {
            const { academicYearStart } = this.parent;
            if (!academicYearStart || !value) return true;
      ...
      preferred_room: Yup.string()
        .max(10, "Бажана кімната: макс. 10 символів")
        .nullable()
        .transform((value) => (value === "" ? null : value)),
      comments: Yup.string()
        .max(1000, "Коментарі: макс. 1000 символів")
        .nullable()
        .transform((value) => (value === "" ? null : value)),
    },
    [["academicYearStart", "academicYearEnd"]]
  ).test('dates-validation', 'Помилка в датах', function (values) {
    const {
      startDay,
      startMonth,
      startYear,
    ...
    if (!startDate && (startDay || startMonth || startYear)) {
      if (startDay || startMonth || startYear)
        errorsList.push(this.createError({ path: 'startDay', message: 'Некоректна дата початку' }));
    }
    if (!endDate && (endDay || endMonth || endYear)) {
      if (endDay || endMonth || endYear)
        errorsList.push(this.createError({ path: 'endDay', message: 'Некоректна дата закінчення' }));
    }
    if (!appDate && (applicationDateDay || applicationDateMonth || applicationDateYear)) {
      if (applicationDateDay || applicationDateMonth || applicationDateYear)
        errorsList.push(this.createError({ path: 'applicationDateDay', message: 'Некоректна дата заяви' }));
    ...
    if (startDate && endDate && startDate > endDate) {
      errorsList.push(
        this.createError({ path: 'endDay', message: 'Дата закінчення має бути після або рівна даті початку' })
      );
    }
