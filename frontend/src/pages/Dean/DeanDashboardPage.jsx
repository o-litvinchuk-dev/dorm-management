...
const DeanDashboardPage = () => {
  ...
  const formatSectionCount = (count, labelPrefix = "", itemTitle = "") => {
    if (isLoading && count === 0 && !itemTitle.includes("Налаштування") && !itemTitle.includes("Розклад")) return "...";
    if (count === '?') return "?";
    if (!labelPrefix && (itemTitle.includes("Налаштування") || itemTitle.includes("Розклад"))) return undefined;
