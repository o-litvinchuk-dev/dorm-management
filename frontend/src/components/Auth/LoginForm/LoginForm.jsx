...
const LoginForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    // Перевірка пароля на клієнті, якщо потрібно
    ...
    try {
      ...
      localStorage.setItem("sessionId", response.data.sessionId); // Зберігаємо sessionId
      await refreshUser();
      ...
      } else if (error.response?.data?.code === "AUTHENTICATION_FAILED") { // Новий код
        ToastService.authFailed();
  ...
  const handleGoogleSignIn = useCallback(async (response) => {
    ...
    try {
      ...
      localStorage.setItem("sessionId", res.data.sessionId); // Зберігаємо sessionId
      await refreshUser();
      navigate("/dashboard");
