...
const DormPassCard = () => {
    ...
    useEffect(() => {
        const fetchPass = async () => {
            ...
            try {
                ...
                // Змінено: Обробляємо 404 як очікуваний випадок, а не помилку
                if (err.response?.status === 404) {
                    setPassData(null); // Просто встановлюємо, що даних немає
                } else {
    ...
    // Змінено: Окремий блок для помилок, щоб не плутати з відсутністю перепустки
    if (error) {
        return (
    ...
    // Змінено: Цей блок тепер коректно відображається при 404
    if (!passData) {
        return (
